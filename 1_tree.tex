...
Define the near-optimal tree $$\Tau_h^\infty = \left\{a\in A^h: V^*-V(a) \leq \frac{\gamma^h}{1-\gamma}\right\}.$$ Introduce the difficulty measure $\kappa$ such that $|\Tau_h^\infty| = \cO(\kappa^h)$ (the smallest).

Then, the action $a_n$ recommended by \texttt{OPD} has a simple regret bounded by:
\begin{align*}
r_n = \cO\left(n^{-\frac{\log 1/\gamma}{\log \kappa}}\right);
\end{align*}
\end{theorem}
...
Let $L \leq V\leq U$ consistent bounds, then planning with $L^a,\,U^a$ gives the following simple regret bound: %$\forall \kappa>\kappa(U)$,
\begin{align*}
