...
A very popular approach is to follow the principle of \emph{Optimism in the Face of Uncertainty} (OFU) \citep{Munos14}, which consists in exploring the option that maximises an upper-bound of the true objective. In the context of planning, it can be applied by forming bounds on the state value function $V$.
...
\paragraph{Confidence bounds for rewards}

When the reward $r\sim \probability{r \condbar s,a}$ is stochastic, deviation inequalities can be used to design a confidence interval $[\ell_t(s,a), u_t(s,a)]$ over its expected value $\mu(s,a) = \expectedvalue\left[r | s,a\right]$. For instance, the confidence interval based on the Chernoff-Hoeffding deviation inequality was first used in \citep{Kocsis06UCT,Bubeck2010open}.
In recent works \citep{Leurent2019practical, Jonsson2020}, a Kullback-Leibler confidence interval is used instead.
\begin{align*}
...
\end{align*}
where $\beta^r$ is an exploration function and $\kl(u,v)$ is the binary Kullback-Leibler divergence between two Bernoulli distributions: $\kl(u,v) = u \log \tfrac {u} {v} + (1-u) \log \tfrac {1-u} {1-v}$.

\paragraph{Confidence bounds for transitions}

A confidence set on the probability vector $p(\cdot|s,a)$ can be defined as
$\cC_t(s,a) \eqdef \left\{p\in \Sigma_{B} :  \KL\!\big(\hp_t(\cdot|s,a),p\big) \leq \frac {\beta^p(n_t(s,a), \delta)} {n_t(s,a)}\right\}$.
...
In the deterministic setting, once a transition has been observed, it is known with no uncertainty. Hence, we only simulate transitions from external nodes $\ext{\cG_n}$. In contrast, in the stochastic setting, we need to estimate expected reward and the transition probabilites from many samples, which implies that internal nodes $\inte{\cG_n}$ must be sampled as well.

Stochastic systems: the approach is to sample trajectories of length $H$, where $H$ can be tuned depending on the budget $n$. For instance, in SS, OLOP..., BRUE, MDP GapeE
...
By running linear regression, we find a slope of $\sigma \simeq-0.04$ for BRUE, KL-OLOP, MDP-GapE, which corresponds to an effective branching factor $\kappa_e \simeq \exp(-\log(1/\gamma)/\sigma) = 3.6$. In contrast, \GBOP has a slope $\sigma \simeq-0.3$, which corresponds to an effective branching factor $\kappa_e \simeq 1.2$.
