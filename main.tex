...
% Algorithm~\ref{alg:OptimisticVIcompl}.f
...
\begin{definition}[Bellman Optimal operator]
	...
	& \text{if $(s,b,s')\in{\cG}_n$.}
	\end{cases}
	\end{equation}
	where $(s,b,s')$ represents the transition (edge) from $s$ to $s'$ through $b$.
...
\begin{figure}[tp]
	...
	\label{fig:bellman}
\end{figure}
\el{il manque $\cB_n$ dans la figure}
...
\begin{definition}[Consistency]
	We say that a pair of bounds ($L$, $U$) or $(\cL, \cU)$ is \emph{consistent} if they are respectively non-decreasing and non-increasing along transitions:
	\begin{align*}
	\forall a\in\inte{\Tau}_n,\quad & L(a) \leq& \max_{b\in A} r(s(a), b) + \gamma L(ab), \quad U(a) &\geq \max_{b\in A} r(s(a), b) + \gamma U(ab)\\
	\forall (s)\in\inte{\cG}_n,\quad & \cL(s) \leq& \max_{(s,b,s')\in\cG_n} r(s, b) + \gamma \cL(s'), \quad \cU(a) &\geq \max_{(s,b,s')\in\cG_n} r(s, b) + \gamma \cU(s')
	\end{align*}
\end{definition}

\el{Find better notations: $T(s,a)$ }

\begin{lemma}[Properties of $B_n$]
	\begin{enumerate}[label=(\roman*)]
		\item $B_n$ preserves consistency and tightens consistent bounds: $$
...
\begin{lemma}[Properties of $B_n$]
	...
	\begin{enumerate}[label=(\roman*)]
		\item $\cB_n$ preserves consistency and tightens consistent bounds: $$
	...
	\end{enumerate}
\end{lemma}
which motives us to propose \Cref{alg:gbopd}, following the approach of \Cref{alg:opd} adapted to a graph structure. Though the two algorithms share a very similar design, we claim that using graphs provides substantial theoretical and practical performance improvements, and back up this statement in \Cref{sec:analysis,sec:experiments}.
...
Comparing Algorithms \ref{alg:opd} and \ref{alg:gbopd} directly is difficult since they do not involve the same structure, which implies implicit differences in their behaviours. Studying them under a common framework would make these differences explicit. In order to leverage the analysis of the \texttt{OPD} algorithm by \citet{Hren2008optimistic}, we frame our \Cref{alg:gbopd} as a tree-based planning algorithm. To account for the distinction between the operators $B$ and $\cB$, we represent $\cB$ as tree backup $B$ augmented with \emph{merge} operator $M$ which enforces that nodes with similar states share the same value.
...
\end{enumerate*}
They are implemented on top of \Cref{alg:opd} through the introduction of two additional operators $M$ and $P$, defined in \Cref{def:merge} and \Cref{prop:pruning}, and illustrated in \Cref{fig:operators}.
...
In \Cref{alg:gbopd}, the bounds $(\cL_n, \cU_n)$ are defined the a fixed-point iteration of $\cB$, starting from the trivial bounds $(0,\, V_{\max})$.  We propose to translate this definition in the context of trees with an alternating procedure of backups $B$ and merges $M$: at iteration $n$ we define the bounds
\begin{equation}
...
\begin{lemma}[Properties of $M_nB_n$]
	...
	\begin{enumerate}[label=(\roman*)]
		\item $M_n B_n$ preserves consistency and tightens consistent bounds;
...
\begin{definition}[Bounds equivalence]
	We say that a bound $U$ defined on $T_n$ is \emph{equivalent} to a bound $\cU$ defined on $\cG_n$ if for all $a\in A^*$, $U(a) = \cU(s(a))$.
\end{definition}
...
\begin{corollary}
	...
	as those ($\hat{s}_n$) sampled by \Cref{alg:gbopd}.
\end{corollary}

We've shown that $MB^\infty$ provides the same candidates for expansion. It remains to 
...
The tree version of \Cref{alg:gbopd}, that we describe in \Cref{alg:gbop-t}.
...
\begin{align*}
u_t(s,a) &\eqdef \max \left\{v : \kl\!\big(\hat{r}_t(s,a),v\big) \leq \frac {\beta^r(n_t(s,a), \delta)} {n_t(s,a)} \right\},\\
\ell_t(s,a) &\eqdef\min \left\{v : \kl\!\big(\hat{r}_t(s,a),v\big) \leq \frac {\beta^r(n_t(s,a), \delta)} {n_t(s,a)} \right\},
\end{align*}
where $\hat{r}_t(s,a)$ is the sample mean, $\beta^r$ is an exploration function and $\kl(u,v)$ is the binary Kullback-Leibler divergence between two Bernoulli distributions: $\kl(u,v) = u \log \tfrac {u} {v} + (1-u) \log \tfrac {1-u} {1-v}$.
...
$\cC_t(s,a) \eqdef \left\{p\in \Sigma_S :  \KL\!\big(\hp_t(\cdot|s,a),p\big) \leq \frac {\beta^p(n_t(s,a), \delta)} {n_t(s,a)}\right\}$,
where $\Sigma_S$ is the probability simplex, $\beta^p$ is an exploration function and $\KL(p,q)= \sum_{s \in \text{Supp}(p)}  p(s) \log \tfrac{ p(s)} {{q}(s)}$ is the Kullback-Leibler divergence between two categorical distributions
...
\end{align*}
for all $s\in\inte{\cG_n}$, where the maximum and minimum over these Kullback-Leibler confidence regions $\cC_t(s,a)$ can be computed as explained in \citep[Appendix A of][]{Filippi2010optimism}. Under the event that every confidence regions $[\ell_t(s,a), u_t(s,a)]$ and $\cC_t(s,a)$ are valid at time $t$, the \Cref{prop:bellman-graph} still holds for $\cB_t^-, \cB_t^+$.

\paragraph{Structure of the planning algorithm}

In the deterministic setting, once a transition has been observed, it is known with certainty and doesn't need to be sampled ever again, which is why only external nodes $\ext{\cG_n}$ are sampled in \Cref{alg:gbopd}. Conversely, in the stochastic setting the expected reward and transition probabilities must be estimated from samples, which implies that internal nodes $\inte{\cG_n}$ must be sampled as well. Then, it is common to adopt and episodic setting where we sample trajectories of a fixed horizon $H$, tuned depending on the budget $n$. This is the case in  \citep[e.g.][]{Kearns02SS,Kocsis06UCT,Bubeck2010open,Feldman14BRUE,Leurent2019practical,MDPGapE2020}.
...
\subsection{Proof of \Cref{lem:contractivity}}
\begin{proof}
Let $U_1, U_2\in \Real^\Tau_n, a\in\Tau_n$,
\begin{align*}
    (M U_1 - M U_2)(a) &= \min_{a'\in N_n(a)} U_1(a') - \min_{a'\in N_n(a)} U_2(a') \\
...
\begin{figure}[H]
	\centering
	% \includegraphics[width=0.49\linewidth]{img/4_updates_deterministic.pdf}
	% \includegraphics[width=0.49\linewidth]{img/4_updates_kl-olop.pdf}
