...
\oam{Q: Is "confidence" the right term here? }
\el{No}
...
Given bounds $L_n, U_n$ available at iteration $n$, this provides an optimistic sampling rule: expand the node
\begin{equation}
...
\el{Example of the trivial bounds, used in OPD. Such bounds $(L_n,\,U_n)$  can also be used to incorporate prior knowledge over the state value. For instance, terminal states can be handled by setting $L_n(a) = U_n(a) = 0$.}

For instance, in \texttt{OPD}, \citet{hren2008optimistic} used the trivial bounds $L=0,\,U=V_{\max}$ and proved the following regret bound.
...
\subsection{Motivation for improving the sample-efficiency}
\oam{for improvement of what? can be more precise}
...
If several sequences $a'\in\Tau_n$ all lead to the same state $s$, their respective bounds must hold simultaneously. This suggests an aggregation operator $\cA$ as:
    ...
    \label{eq:aggregation}
        \forall a\in\mathcal{T}_n, \qquad &\cA_n^-(L)(a) = \max_{a'\in \cN_n(a)} L(a'),\qquad \cA_n^+(U)(a) = \min_{a'\in \cN_n(a)} U(a').                                   
    \end{align}
\end{definition}
...
\begin{algorithm}[H]
	...
	\For{iteration $n$}{
		\nl Given $\Tau_n$, compute $L_n = (\cA_n^-\cB_n)^\infty(0)$ and $U_n = (\cA_n^+\cB_n)^\infty(V_{\max})$ with \eqref{eq:aggregation} and \eqref{eq:bellman}.\;
