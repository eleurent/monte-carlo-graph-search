...
\author{\Name{Edouard Leurent} \Email{edouard.leurent@inria.fr}\\
	\Name{Odalric-Ambrym Maillard} \Email{odalric.maillard@inria.fr}\\
...
\end{align*}
for all $s\in\inte{\cG_n}$, where the extremums over these KL confidence regions $\cC_t(s,a)$ can be computed as explained in \citep[][Appendix A]{Filippi2010optimism}. Under the event that all confidence regions $[\ell_t(s,a), u_t(s,a)]$ and $\cC_t(s,a)$ are valid, the Lemma \ref{lem:properties-b-graph} still holds for $\cB_t^-, \cB_t^+$.

\paragraph{Structure of the planning algorithm}

In the deterministic setting, once a transition has been observed, it is known with certainty and doesn't need to be sampled ever again, which is why only external nodes $\ext{\cG_n}$ are sampled in \GBOPD. Conversely, in the stochastic setting the expected reward and transition probabilities must be estimated from samples, which implies that internal nodes $\inte{\cG_n}$ must be sampled as well. Then, it is common to adopt and episodic setting where we sample trajectories of a fixed horizon $H$, tuned depending on the budget $n$. This is the case in  \citep[e.g.][]{Kearns02SS,Kocsis06UCT,Bubeck2010open,Feldman14BRUE,Leurent2019practical,Jonsson2020planning}. We also follow this scheme in our proposed \GBOP algorithm.
...
We thank Mathieu Seurin and {\'E}milie Kaufmann for their valuable comments.
...
Thus, we observe that there is a \emph{trade-off} between the time complexity $k(\varepsilon)$ and the sample complexity $\hlob{\hat{\kappa}_\infty}$: decreasing one increases the other. We illustrate this trade-off empirically in \Cref{fig:approximation}.
...
\begin{figure}
\begin{center}
\includegraphics[trim={1.8cm 0.4cm 1.8cm 0.4cm}, clip, width=0.24\linewidth]{img/epsilon/updates_GBOP-D_1e-2}
\includegraphics[trim={1.8cm 0.4cm 1.8cm 0.4cm}, clip, width=0.24\linewidth]{img/epsilon/updates_GBOP-D_1e0}
\includegraphics[trim={1.8cm 0.4cm 1.8cm 0.4cm}, clip, width=0.24\linewidth]{img/epsilon/updates_GBOP-D_1e1}
\includegraphics[trim={1.8cm 0.4cm 1.8cm 0.4cm}, clip, width=0.24\linewidth]{img/epsilon/updates_GBOP-D_1e2}\\
\includegraphics[trim={1.8cm 0.4cm 1.8cm 0.4cm}, clip, width=0.24\linewidth]{img/epsilon/occupations_GBOP-D_1e-2}
\includegraphics[trim={1.8cm 0.4cm 1.8cm 0.4cm}, clip, width=0.24\linewidth]{img/epsilon/occupations_GBOP-D_1e0}
\includegraphics[trim={1.8cm 0.4cm 1.8cm 0.4cm}, clip, width=0.24\linewidth]{img/epsilon/occupations_GBOP-D_1e1}
\includegraphics[trim={1.8cm 0.4cm 1.8cm 0.4cm}, clip, width=0.24\linewidth]{img/epsilon/occupations_GBOP-D_1e2}
\end{center}
\caption{\textbf{Top row}: Map showing the number of updates triggered by $\cB$ at each expanded state, and for various values of $\varepsilon$. \textbf{Bottom row}: Map showing corresponding. The rightmost column corresponds to $\epsilon \geq V_{\max} = 20$, \emph{i.e.} every approximation stops after a single iteration.}
...
\end{enumerate}
These idea are formalised in \Cref{alg:queue-b-inf}. Note that the criterion $\|\cU_n^{k+1} - \cU_n^k\| \leq \frac{1-\gamma}{\gamma}\varepsilon$ is used to detect that the limit $\cU_n^\infty$ is approximated with accuracy $\varepsilon$, and stems from $\cB_n$ being a $\gamma$-contraction:
\begin{proof}
$\|\cU_n^k - \cU_n^\infty\| \leq \gamma\| \cU_n^{k+1} - \cU_n^\infty\| \leq \gamma \|\cU_n^{k+1} - \cU_n^{k}\| + \gamma \|\cU_n^{k} - \cU_n^\infty\|$, with $\| \cU_n^{k+1} - \cU_n^{k}\| \leq \frac{1-\gamma}{\gamma}\varepsilon$, thus $\|\cU_n^{k} - \cU_n^\infty\| \leq \varepsilon$.
\qed\end{proof}
...
\begin{figure}[tp]
	\centering
	\includegraphics[trim={0 0.5cm 0 0.5cm}, clip, width=0.65\linewidth]{img/tree_OPD.pdf}
	\includegraphics[trim={0 0.5cm 0 0.5cm}, clip, width=0.65\linewidth]{img/tree_KL-OLOP.pdf}
	\includegraphics[trim={0 0.5cm 0 0.5cm}, clip, width=0.65\linewidth]{img/tree_GBOP-D.pdf}
