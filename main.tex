...
\begin{definition}[Monotonicity]
	...
	\forall (s)\in\inte{\cG}_n,\quad & \cL(s) \leq& \max_{b\in A} r(s, b) + \gamma \cL(T(s,b)), \quad \cU(s) &\geq \max_{b\in A} r(s, b) + \gamma \cU(T(s,b))
	\end{align*}
\end{definition}
...
Thus, this result shows that \Cref{thm:regret-state-aware} is non-trivial since we exhibit a problem where $\hlgb{\kappa_\infty} < \hlrb{\kappa}$ (when $K\geq 3$), and legitimates our attempt to improve planning performances by merging the tree into a graph.
