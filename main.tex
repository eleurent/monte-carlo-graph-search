...
\begin{figure}[htp]
	\centering
	\includegraphics[trim={2.2cm 0 2.5cm 0}, clip, width=0.44\textwidth]{img/tree_5.pdf}
	\caption{The tree $T(\cG_n)$ obtained by unrolling $\cG_n$. Contrary to $T_n$ shown in \Cref{fig:structures}, the red leaf has been expanded at the same time as the internal red node, which allows to tighten its value bounds.}
	\label{fig:unroll}
\end{figure}

Equivalently, after expanding a leaf in $T_n$, we simultaneously expand any leaf that leads to the same state as an internal node. These expansion can only tighten the bounds at these leafs.
...
Then, for any sequence of action $a\in T(\cG_n)$, we have $L_n(a) = \cL_n(s(a))$ and $U_n(a) = \cU_n(s(a))$.


\begin{lemma}[Bounds of sequence values]
	The bounds $(\overline{L}_n, \overline{U}_n)$ on the value of sequences of actions verify:
	\begin{enumerate}[label=(\roman*)]
		\item $\overline{L}_n$ is non-decreasing with $n$ and $\overline{U}_n$ is non-increasing with $n$.
		\item For all $a\in A$, $\overline{U}(a) = \max_{a'\in a A^\infty} \overline{U}(a')$.
	\end{enumerate}
\end{lemma}

In this context, the sampling rules are and recommendation rules of \GBOPD are similar to those of \OPD applied to the tree $T(\cG_n)$, except that the sampled node $b_n$ and recommended nodes $a_n$ belong to $T(\cG_n)$ and thus can have an infinite depth. 

\begin{lemma}[Expansion]
	\label{lemma:expansion-bound-2}
	Let $a$, a node of depth $h$ traversed by the optimistic sampling rule of \GBOPD at iteration $t$. Then its first action $a_1$ enjoys a simple regret 
	\begin{equation}
	\label{eq:expansion-regret}
	V(a^\star)-V(a_1) \leq \gamma^h(U_t(a)-L_t(a)).
	\end{equation}
