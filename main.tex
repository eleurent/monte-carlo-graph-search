...
\end{enumerate*}
The \Cref{sec:implementation} discusses an approximate implementation where these procedures are stopped whenever they reach a desired accuracy $\varepsilon$. This  $\varepsilon$ can be tuned with respect to the budget $n$.
\end{remark}
...
The near-optimal branching factor $\hlrb{\kappa}$ is related [ref OLOP] to the near-optimality dimension studied in the online optimisation literature \citep[see e.g.][]{DOO HOO SOO POO}. % TODO
...
\begin{figure}[htp]
	...
	\caption{The tree $T(\cG_n)$ obtained by unrolling $\cG_n$. Contrary to $T_n$ shown in \Cref{fig:structures}, the red leaf is expanded at the same time as the internal red node, which enables to tighten its value bounds by applying $B_n$.}
	\label{fig:unroll}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{tabular}{cccccc}
		...
		$k$ & $0$ & $1$ & $\cdots$ & $k$ \\
		\midrule
		$\cB^k(\cU)(s)$ & $V_{\max}$ & $\frac{1}{2} + \gamma V_{\max}$ && $\frac{1}{2}(1-\gamma^k)V_{\max} + \gamma^k V_{\max}$\\
		$\cB^k(\cL)(s)$ & $0$ & $\frac{1}{2}$ && $\frac{1}{2}(1-\gamma^k)V_{\max}$\\
		\bottomrule
	\end{tabular}
	\caption{\textbf{Top}: a simple looping MDP with $|S|=|A|=1$ after having observed a single transition ($n=1$). \textbf{Bottom}: the sequence of bounds $(U_1^k)$ when alternating $M$ and $B$. $(U_1^k)$ and $(L_1^k)$ converge geometrically to their limit $V = \frac{1}{2}V_{\max}$, thus in infinite time.}
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{\linewidth}
		...
		\includegraphics[width=0.49\linewidth]{img/epsilon/1e-2/occupations_state_aware.pdf}
		\caption{$\varepsilon=1e-1$}
	\end{subfigure}
	\caption{Updates and occupancies for various values of $\varepsilon$, for $n = 5460$, $\gamma=0.95$}
	\label{fig:epsilon_1}
\end{figure}
\begin{figure}[H]
	\ContinuedFloat
