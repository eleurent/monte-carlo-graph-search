...
In this work, we will be comparing two data structures for planning in an MDP: the tree and the (directed) graph, represented in \autoref{fig:structures}. In order to differentiate them, we use Roman symbols when referring to the trees, e.g. $T, U, L, B$ and calligraphic symbols when referring to the graphs, e.g. $\cG, \cU, \cL, \cB$.
...
\end{enumerate}
The pseudo-code of generic planning algorithm is given in \autoref{alg:generic}.
\begin{algorithm}
...
\begin{definition}[Bellman Optimal operator]
	...
	The updates of both Bellman operators are depicted in \autoref{fig:bellman}.
\end{definition}
...
\begin{figure}[tp]
	...
	\caption{Depiction of the Bellman backup operators $B$ and $\cB$. Notice that $B$ only propagates information upward in the tree.}
	\label{fig:bellman}
\end{figure}
...
\begin{definition}[Consistency]
	...
	\begin{align*}
	\forall a\in\inte{\Tau}_n, \quad &L(a) \leq \max_{b\in A} r(ab) + \gamma L(ab)\\
...
The corresponding \texttt{OPD} algorithm is described in \autoref{alg:opd}. Likewise, we propose \autoref{alg:gbop} which follows the same approach adapted to a graph structure. Though the two algorithms look very similar in their design, we claim that using graphs provides substantial theoretical and practical performance improvements, which we analyse in the following section.
...
\begin{algorithm}[th]
	\caption{Our \emph{Graph-Based Optimistic Planning} algorithm.}
	\label{alg:gbop}
	\DontPrintSemicolon
	\For{each iteration $n$}{
...
Comparing Algorithms \ref{alg:opd} and \ref{alg:gbop} directly is difficult since they do not involve the same structures. Consequently, the effect of the structures on the resulting behaviour is implicit. Thus, we need to compare the two algorithms in a common framework to make these differences explicit. In order to leverage the previous analyses of the \texttt{OPD} algorithm, we frame our \autoref{alg:gbop} as a tree-based planning algorithm. To account for the difference between the operators $B$ and $\cB$ used to compute the bounds, we represent $\cB$ as tree backup $B$ augmented with \emph{merge} operator $M$ which enforces that nodes with similar states share the same value.

\subsection{Background on the sample complexity of \OPD}

First, we recall the analysis of \OPD. We start by introducing some notations which facilitate the analysis.
...
This enables to define a measure of a planning problem difficulty.

\begin{definition}[Difficulty measure]
We define the problem-dependent measure $\hlrb{\kappa}$ as$$\hlrb{\kappa = \limsup_{h\rightarrow\infty} |\hlrb{\Tau_h^\infty}|^{1/h}} \in [1, K] $$
...
One can easily see that, since the $(L_n,\,U_n)$ used in the optimistic sampling rule described in \autoref{alg:opd} are invariant by $B_n$ by definition, this rule can be equivalently expressed as:
\begin{equation}
...
As shown in \autoref{fig:bellman}, in a tree the Bellman operator $B_n$ only propagates the information upward, and the leaves cannot be updated. Thus, $U_n = B_n^{\infty}(V_{\max})$ and $V_{\max}$ coincide on $\ext{\Tau}_n$ which means that the sampling rule of \texttt{OPD} can be summarized as using \eqref{eq:sampling_rule} with the trivial upper-bound $U_n = V_{\max}$.
...
\begin{remark} For ease of notation, we sometimes refer to $M^-$ and $M^+$ as a single merge operator $M$, that acts as $M^-$ on lower-bounds and $M^+$ on upper-bounds.
\end{remark}
...
\begin{figure}[H]
	...
	\caption{Depiction of the merge operator $M$. Nodes can be updated anywhere in the tree $\Tau_n$, and in particular its leaves $\ext{\Tau}_n$.}
\end{figure}

\begin{lemma}[Properties of $M$]
	\begin{enumerate}[(i)]
		\item $M^2=M$;
		\item $M$ preserves consistency;
		\item $M$ tightens bounds:
		$
...
We propose that at each step, we detect such pairs $a_1, a_2$. Whenever $a_1$ is a leaf, we can remove it from the set $\ext{\Tau}_n^-\subset \ext{\Tau}_n $ of candidates for expansion.
...
\begin{algorithm}[H]
	\caption{Tree version of \autoref{alg:opd}}
	...
	\For{iteration $n$}{
		...
		Prune the tree: $\ext{\Tau}_{n+1}^- = \cP(\ext{\Tau}_{n+1})$.\;
	}
...
\end{align*}
with $\hlgb{\kappa_\infty} \leq \hlrb{\kappa}$.
\end{theorem}

Intuitively, $\kappa_\infty$ will be much lower than $\kappa$ in problems where trajectories overlap a lot. For instance it is the case with the class of problems where two actions cancel each-other out (e.g. moving left or right), or when the outcome of several actions does not depend on the order (e.g. placing pawns on a board game).

\subsection{Illustrative example}
\label{sec:illustrative-example}
We consider a toy MDP $\cM$ shown in \autoref{fig:mdp}. The transitions are described visually while the rewards are defined as follows: let $0\leq r^\star\leq \gamma$, and $ r^- = r^\star - \frac{\gamma}{1-\gamma} S$, $r^+ = r^\star + S$ with $S = r^\star\left(\frac{1}{\gamma} - 1\right).$ Note that the choice ensures that $r^\star, r^-, r^+$ and $S$ are all in $[0, 1]$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.44\textwidth]{img/4_deterministic.pdf}
    \includegraphics[width=0.44\textwidth]{img/4_kl-olop.pdf}
    \includegraphics[width=0.44\textwidth]{img/4_state_aware.pdf}
    \caption{Trees expanded for $n = 5460$, $\gamma=0.95$}
    \label{fig:gw4_trees}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.44\textwidth]{img/4_occupations_deterministic.pdf}
    \includegraphics[width=0.44\textwidth]{img/4_occupations_kl-olop.pdf}
    \includegraphics[width=0.44\textwidth]{img/4_occupations_state_aware.pdf}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.44\textwidth]{img/4_updates_state_aware.pdf}
...
The \autoref{fig:mdp-tree} shows the planning tree corresponding to the MDP $\cM$. Whenever the action $a_1$ is taken \textcolor{OliveGreen}{(in green)} the resulting subtree is represented by a leaf node $s^\star$ of value $V^\star = \frac{r^\star}{1-\gamma}$. When, in contrast, we take a sequence of actions among $a_2\dots a_K$ \textcolor{red}{(in red)}, we stay in the state $s^+$ and denote $V_h$ the corresponding value at depth $h$.
