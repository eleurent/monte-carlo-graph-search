...
\begin{proof}
	\el{TODO}
	 Since $U_t=B_t(U_t)$ and $L_t=B_t(L_t)$, 
\qed\end{proof}
...
$\cB_n^\infty$ can converge in infinite time whenever $\cG_n$ contains a loop, as shown in \Cref{fig:simple_loop}. We consider the effect of stopping early after a fixed number of iterations $k(\varepsilon,\gamma)$.
...
The sampling rule of \GBOPD (line 2) can take yield to an infinite sequence $b_n$. We propose to stop the sampling after a fixed depth $d^+_n$.

\begin{proposition}[Time complexity of sampling]
	Consider the variant of \GBOPD where we stop the sampling rule when reaching a fixed depth $d^+_n$ that is chosen polynomial with $n$:
...
\subsection{Partial Bellman update $\cB_n$}
