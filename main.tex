...
In a second experiment, we consider a the sailing domain [ref]: a boat can move in $K=8$ directions, and endures a cost (move duration) depending on the direction of the wind which follows stochastic dynamics. The \Cref{fig:sailing} shows the evolution of the simple regret $r_n$ of stochastic planning algorithms with respect to the number $n$ of oracle calls. The slope $\sigma$ of $r_n$ provides an empirical measurement of the effective branching factor $\kappa_e = \exp(-\log(1/\gamma)/\sigma)$ for each algorithms. A linear regression gives $\sigma \simeq-0.04$ and $\kappa_e \simeq = 3.6$ for BRUE, KL-OLOP, MDP-GapE. In contrast, we measure $\sigma \simeq-0.25$ and $\kappa_e \simeq 1.2$ for \GBOP, which suggests that our result of \Cref{thm:regret-gbop-t} might generalize to the stochastic setting.
