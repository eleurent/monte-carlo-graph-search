...
\begin{algorithm}[th]
	...
	\For{each iteration $n$}{
		...
		\For(\Comment*[f]{Node expansion}){action $a\in A$}{
			Simulate $r, s' \sim \probability{r, s' \condbar s(b_n), a}$.\;
...
\begin{algorithm}[th]
	...
	\For{each iteration $n$}{
		...
		\nl \While{$s_n\in\inte{\cG}_n$}{
			$s_n\gets \displaystyle\argmax_{s'} r(s, a) + \gamma \cU_n(s')$ \Comment*[r]{Optimistic sampling rule}
		}
		\For(\Comment*[f]{Node expansion}){action $a\in A$}{
			Simulate $r, s' \sim \probability{r, s' \condbar s_n, a}$.\;
...
\end{enumerate*}
The \Cref{sec:implementation} discusses an approximate implementation where these procedures are stopped whenever they reach a desired accuracy $\varepsilon$, along with the corresponding time complexity analysis. The accuracy $\varepsilon$ can be tuned with respect to the budget $n$ to preserve the theoretical guarantees of the algorithm.
\end{remark}

Though the two algorithms share a similar design, we claim that using graphs provides substantial theoretical and practical performance improvements, and back up this statement in \Cref{sec:analysis,sec:experiments}.
...
The near-optimal branching factor $\hlrb{\kappa}$ is related [ref OLOP] to the near-optimality dimension studied in the online optimisation literature \citep[see e.g.][]{DOO HOO SOO POO}. \todo{TODO}
...
\begin{equation}
T(\cG_n) = \{a\in A^h: s_t \in \cG_n \text{ with } s_{t+1} = T(s_{t}, a_t)\text{ for } 0 \leq t < h\}
\end{equation}
...
\begin{lemma}[Expansion]
	...
	Let a node $a$ of depth $h$ traversed by the optimistic sampling rule of \GBOPD at iteration $n$. Then, its first action $a_1$ enjoys a simple regret 
	\begin{equation}
	\label{eq:expansion-regret}
	V(a^\star)-V(a_1) \leq \gamma^h(U_n(a)-L_n(a)).
	\end{equation}
	
	In particular, if the sampling rule samples an infinite sequence $a\in A^\infty$, its first action $a_1$ is optimal, and we write that \eqref{alg:gbop-d} also holds for $a$ with $h=\infty$.
\end{lemma}
...
% \end{enumerate}
\end{proposition}
This result confirms that \Cref{thm:regret-gbop-t} is non-trivial since we exhibit a problem for which $\hlgb{\kappa_\infty} < \hlrb{\kappa}$ (when $K\geq 3$), and legitimates our attempt to improve planning performances by merging the tree into a graph.
...
\begin{algorithm}[ht]
	...
	\For{trajectory $m$ in $[1, M]$}{
		\For{time $t$ in $[1, H]$}{
			...
			Simulate $r_t, s_{t+1} \sim \probability{r, s_{t+1} \condbar s_t, b_t}$.\;
...
\begin{lemma}[Bounds of sequence values]
	The bounds $(\overline{L}_n, \overline{U}_n)$ on the value of sequences of actions verify:
	\begin{enumerate}[label=(\roman*)]
		\item $\overline{L}_n$ and $\overline{U}_n$ are respectively non-decreasing and non-increasing with respect to $n$.
		\item For all $a\in A^*$, $\overline{U_n}(a) = \max_{a'\in a A^\infty} \overline{U}(a')$.
	\end{enumerate}
\end{lemma}

Which enables us to proceed to the proof of \Cref{lem:expansion-bound}.
\begin{proof}
	Let $t$ be the time of expansion of $a$, it holds that $\overline{U}_t(b) \leq \overline{U}_t(a)$ for all $b\in T(\cG_n)$, in particular those in a branch starting by an optimal action $a^\star$. Since $U_t=B_t(U_t)$ and $L_t=B_t(L_t)$, we also have $\overline{U}_t(a^\star) = \max_{b\in a^\star A^*} \overline{U}_t(b) \leq \overline{U}_t(a)$, and $\overline{L}_t(a_1) = \max{b\in a_1 A^*} \overline{L}_t(b) \geq  \overline{L}_t(a)$. Thus, $V(a^\star)-V(a_1) \leq \overline{U}_t(a^\star) - \overline{L}_t(a_1) \leq \overline{U}_t(a) - \overline{L}_t(a) = \gamma^h(U_t(a)-L_t(a))$.
\qed\end{proof}

\subsection{Proof of \Cref{lem:recommendation-bound}}
\begin{proof}
	Let $i$ an expanded node of maximal depth $d_n\in\Real\cup\{\infty\}$, and consider the recommended node $a_n$ at time $n$, of depth $d\in\Real\cup\{\infty\}$. In particular, $\overline{L}_n(a_n) \geq \overline{L}_n(i)$, and since $(\overline{L}_t)_t$ is non-decreasing we also have $\overline{L}_n(i) \geq \overline{L}_t(i)$. At the time $t$ when $i$ is expanded, we have $\overline{U}_t(a_n) \leq \overline{U}_t(i)$, and since $(\overline{U}_t)_t$ is non-increasing we also have $\overline{U}_n(a_n) \leq \overline{U}_t(a_n)$. We can conclude with \Cref{lem:expansion-bound-U-2} applied to $a_n$: $r_n \leq \gamma^d(U(a_n)-L(a_n) = \overline{U}_n(a_n) - \overline{L}_n(a_n)  \leq \overline{U}_t(a_n) - \overline{L}_n(i) \leq \overline{U}_t(i) - \overline{L}_t(i) = \gamma^{d_n}(U_t(i) - L_t(i)$, which yields the claimed bound since $U(i) - L(i) \leq V_{\max}-0$.
\qed\end{proof}
...
We can show that at each iteration $n$ the expanded node must belong to $\Tau^\infty(L_n,U_n)$, in the same way as \Cref{lem:near-optimal-nodes-U}.
Let $n\geq n_0$, and define $d_0 = \min\{d\in\Natural: \exists t \in[n_0,n], b_t\in A^d \}$. By definition, for all $d\geq d_0$, any expanded node of depth $d$ was expanded at a time $t\geq n_0$, and thus $b_t\in\Tau^\infty_t \subset\Tau^\infty_{n_0}$. By denoting $n_d$ the number of expanded nodes of depth $d$, we obtain:
\[
...
Moreover, given a history of observed transitions up to iteration $n$, the bounds $U_n, L_n$ obtained from \eqref{eq:gbop-t-bounds} on the unrolled tree $T(\cG_n)$ are tighter than those of \eqref{eq:opd-bounds} since $T_n\subset T(\cG_n)$, which imply by \Cref{lem:shrink} that $\kappa(L_n, U_n) \leq \kappa$. We obtain $\kappa_\infty \leq \kappa$ at the limit. 
\qed\end{proof}
...
$\cB_n^\infty$ can converge in infinite time whenever $\cG_n$ contains a loop, as shown in \Cref{fig:simple_loop}
...
\begin{proof}
	...
	\begin{align*}
	|\overline{U}_{k+1}(a) - \overline{U}_{k}(a)| &\leq \gamma^h\varepsilon (1-\gamma)\gamma^{-h-1} = \varepsilon (1-\gamma)\gamma^{-1}
	\end{align*}
...
The number of iterations $k(\epsilon,\gamma)$ required to compute $\cL_n$, $\cU_n$ with an accuracy $\varepsilon$ is $$k(\epsilon,\gamma) = \log_\gamma\frac{1}{\varepsilon(1-\gamma)}.$$ 
\end{proposition}

\el{Number of udpates vs epsilon}


\subsection{Lazy Bellman update $\cB_n$}
...
\el{implementation of $\cB^\infty$}

\section{Experiments}

\subsection{Parameters}

Since the rewards are deterministic and the transitions are stochastic, we used $\beta^r(n_t(s,a), n) = 0$ and $\beta^p(n_t(s,a), n) = \log n$.
...
We compare the tree $T_n$ expanded by \OPD to the unrolled graph $T(\cG_n)$ of \GBOPD in the deterministic gridworld domain. For clarity of the figure, we only display the nodes selected for expansion and not the automatic expansions as in \Cref{fig:unroll}. \Cref{fig:trees_expanded} shows that \OPD explored uniformly in the space of sequences, which results in contentrated exploration in the state space as seen in \Cref{fig:deterministic-gridworld}. Conversely, \GBOPD expands a sparse unbalanced tree which actually corresponds to uniform exploration in the state space, and allows to explore much deeper for the some budget $n$.
